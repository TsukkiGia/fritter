...
const doesDownvoteExist = async (req: Request, res: Response, next: NextFunction) => {
  ...
  if (!downvote) {
    res.status(400).json({
      error: `Current user has not downvoted Freet with freet ID ${req.query.freetId as string}`
    });
    return;
  }
...
const doesDownvoteNotExist = async (req: Request, res: Response, next: NextFunction) => {
  ...
  if (downvote) {
    res.status(400).json({
      error: `Current user has already downvoted Freet with freet ID ${req.body.freetId as string}`
    });
    return;
  }
